...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component sample_dataset")
      println("exiting now...")
        exit 1
    }
  }
...
def outFromIn(_params) {
  ...
  _params
    ...
    .collect{ it ->
      ...
      def newName =
        (id != "")
          ? id + "." + "sample_dataset" + "." + extOrName
          : "sample_dataset" + "." + extOrName
      it + [ value : newName ]
    }
...
process sample_dataset_process {
  ...
  script:
    if (params.test)
      ...
      ./${params.sample_dataset.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow sample_dataset {
  ...
  result_ = sample_dataset_process(id_input_output_function_cli_params_) \
...
workflow {
  ...
  def id = params.id
  def _params = argumentsAsList(params.sample_dataset) + [ "id" : id ]
  def p = _params
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.sample_dataset.output = "sample_dataset.log"

  Channel.from(rootDir) \
    | filter { params.sample_dataset.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.sample_dataset.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | sample_dataset
