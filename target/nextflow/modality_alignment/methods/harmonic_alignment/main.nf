...
thisHelpMessage = '''harmonic_alignment main_build

Run Harmonic Alignment
...
`ad.obsm['mode2']`.
    ...
    --n_eigenvectors
        type: integer
        default: 100
        Number of eigenvectors of the normalized Laplacian on which to perform
alignment.'''
...
par = {
  ...
  'n_pca_XY': $( if [ ! -z ${VIASH_PAR_N_PCA_XY+x} ]; then echo "int('${VIASH_PAR_N_PCA_XY//\\'/\\\\\\'}')"; else echo None; fi ),
  'n_eigenvectors': $( if [ ! -z ${VIASH_PAR_N_EIGENVECTORS+x} ]; then echo "int('${VIASH_PAR_N_EIGENVECTORS//\\'/\\\\\\'}')"; else echo None; fi )
}
meta = {
...
thisDefaultProcessArgs = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "openproblems-bio/modality_alignment/methods_harmonic_alignment",
    "tag" : "main_build"
  }
}"""),
  // auto settings
