...
nextflow run . \
  -main-script target/nextflow/common/workflows/get_results/main.nf \
  -profile docker \
  -resume \
  --id "get_results_test" \
  ...
  --output_scores "$DATASETS_DIR/results.json"\
  --output_method_info "$DATASETS_DIR/method_info.json"\
  --output_metric_info "$DATASETS_DIR/metric_info.json"\
  --output_dataset_info "$DATASETS_DIR/dataset_info.json"\
  --output_task_info "$DATASETS_DIR/task_info.json" \
