...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component download_datasets")
      println("exiting now...")
        exit 1
    }
  }
...
def outFromIn(_params) {
  ...
  _params
    ...
    .collect{ it ->
      ...
      def newName =
        (id != "")
          ? id + "." + "download_datasets" + "." + extOrName
          : "download_datasets" + "." + extOrName
      it + [ value : newName ]
    }
...
process download_datasets_process {
  ...
  script:
    if (params.test)
      ...
      ./${params.download_datasets.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow download_datasets {
  ...
  result_ = download_datasets_process(id_input_output_function_cli_params_) \
...
workflow {
  ...
  def id = params.id
  def _params = argumentsAsList(params.download_datasets) + [ "id" : id ]
  def p = _params
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.download_datasets.output = "download_datasets.log"

  Channel.from(rootDir) \
    | filter { params.download_datasets.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.download_datasets.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | download_datasets

  emit:
  download_datasets.out
}