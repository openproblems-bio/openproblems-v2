  ...
  "resources" : [
    {
      ...
      "path" : "script.sh",
      "is_executable" : true,
      "parent" : "file:/home/runner/work/openproblems-v2/openproblems-v2/src/common/sync_test_resources/config.vsh.yaml"
    }
  ],
...
$( if [ ! -z ${VIASH_PAR_QUIET+x} ]; then echo "${VIASH_PAR_QUIET}" | sed "s#'#'\\"'\\"'#g;s#.*#par_quiet='&'#" ; fi )
$( if [ ! -z ${VIASH_PAR_DRYRUN+x} ]; then echo "${VIASH_PAR_DRYRUN}" | sed "s#'#'\\"'\\"'#g;s#.*#par_dryrun='&'#" ; fi )
$( if [ ! -z ${VIASH_PAR_DELETE+x} ]; then echo "${VIASH_PAR_DELETE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_delete='&'#" ; fi )
$( if [ ! -z ${VIASH_PAR_EXCLUDE+x} ]; then echo "${VIASH_PAR_EXCLUDE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_exclude='&'#" ; fi )
...
thisDefaultProcessArgs = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "openproblems-bio/common_sync_test_resources",
    "tag" : "main_build"
  }
}'''),
  // auto settings
