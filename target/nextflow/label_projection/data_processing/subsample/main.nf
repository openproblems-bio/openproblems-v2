...
thisHelpMessage = '''subsample main_build

Component to generate a toy data for tests finality

Options:
    --input
        type: file, required parameter
        Input data to be resized

    --celltype_categories
        type: integer, multiple values allowed
        Categories indexes to be selected
...
par = {
  ...
  'celltype_categories': $( if [ ! -z ${VIASH_PAR_CELLTYPE_CATEGORIES+x} ]; then echo "list(map(int, '${VIASH_PAR_CELLTYPE_CATEGORIES//\\'/\\\\\\'}'.split(':')))"; else echo None; fi ),
  'tech_categories': $( if [ ! -z ${VIASH_PAR_TECH_CATEGORIES+x} ]; then echo "list(map(int, '${VIASH_PAR_TECH_CATEGORIES//\\'/\\\\\\'}'.split(':')))"; else echo None; fi ),
...
thisDefaultProcessArgs = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "openproblems-bio/label_projection/data_processing_subsample",
    "tag" : "main_build"
  }
}"""),
  // auto settings
