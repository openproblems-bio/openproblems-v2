...
thisHelpMessage = '''knn_auc main_build
...
`ad.obsm['mode2_aligned']`
    ...
    --proportion_neighbors
        type: double
        default: 0.1
        The propotion of neighbours to use in computing the KNN.

    --n_svd
        type: integer
        default: 100
        The maximum number of SVDs to use.'''
...
par = {
  ...
  'proportion_neighbors': $( if [ ! -z ${VIASH_PAR_PROPORTION_NEIGHBORS+x} ]; then echo "float('${VIASH_PAR_PROPORTION_NEIGHBORS//\\'/\\\\\\'}')"; else echo None; fi ),
...
thisDefaultProcessArgs = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "openproblems-bio/modality_alignment/metrics_knn_auc",
    "tag" : "main_build"
  }
}"""),
  // auto settings
