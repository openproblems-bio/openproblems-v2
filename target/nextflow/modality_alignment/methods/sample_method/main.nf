...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component sample_method")
      println("exiting now...")
        exit 1
    }
  }
...
def outFromIn(_params) {
  ...
  _params
    ...
    .collect{ it ->
      ...
      def newName =
        (id != "")
          ? id + "." + "sample_method" + "." + extOrName
          : "sample_method" + "." + extOrName
      it + [ value : newName ]
    }
...
process sample_method_process {
  ...
  script:
    if (params.test)
      ...
      ./${params.sample_method.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow sample_method {
  ...
  result_ = sample_method_process(id_input_output_function_cli_params_) \
...
workflow {
  ...
  def id = params.id
  def _params = argumentsAsList(params.sample_method) + [ "id" : id ]
  def p = _params
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.sample_method.output = "sample_method.log"

  Channel.from(rootDir) \
    | filter { params.sample_method.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.sample_method.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | sample_method
