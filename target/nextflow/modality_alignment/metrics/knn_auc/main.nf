...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component knn_auc")
      println("exiting now...")
        exit 1
    }
  }
...
process knn_auc_process {
  ...
  script:
    if (params.test)
      ...
      ./${params.knn_auc.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow knn_auc {
  ...
  result_ = knn_auc_process(id_input_output_function_cli_params_) \
...
workflow test {
  ...
  main:
  params.test = true
  params.knn_auc.output = "knn_auc.log"

  Channel.from(rootDir) \
    | filter { params.knn_auc.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.knn_auc.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | knn_auc
