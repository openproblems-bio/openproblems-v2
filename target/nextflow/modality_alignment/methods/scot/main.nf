...
process scot_process {
  ...
  script:
    if (params.test)
      ...
      ./${params.scot.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow scot {
  ...
  result_ = scot_process(id_input_output_function_cli_params_) \
...
workflow {
  ...
  def id = params.id
  def _params = argumentsAsList(params.scot) + [ "id" : id ]
  def p = _params
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.scot.output = "scot.log"

  Channel.from(rootDir) \
    | filter { params.scot.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.scot.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | scot
