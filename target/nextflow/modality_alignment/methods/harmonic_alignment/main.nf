...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component harmonic_alignment")
      println("exiting now...")
        exit 1
    }
  }
...
def outFromIn(_params) {
  ...
  _params
    ...
    .collect{ it ->
      ...
      def newName =
        (id != "")
          ? id + "." + "harmonic_alignment" + "." + extOrName
          : "harmonic_alignment" + "." + extOrName
      it + [ value : newName ]
    }
...
process harmonic_alignment_process {
  ...
  script:
    if (params.test)
      ...
      ./${params.harmonic_alignment.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow harmonic_alignment {
  ...
  result_ = harmonic_alignment_process(id_input_output_function_cli_params_) \
...
workflow {
  ...
  def id = params.id
  def _params = argumentsAsList(params.harmonic_alignment) + [ "id" : id ]
  def p = _params
    .arguments
...
workflow test {
  ...
  main:
  params.test = true
  params.harmonic_alignment.output = "harmonic_alignment.log"

  Channel.from(rootDir) \
    | filter { params.harmonic_alignment.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.harmonic_alignment.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | harmonic_alignment

  emit:
  harmonic_alignment.out
}